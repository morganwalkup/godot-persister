class_name SettingsMigrations
extends RefCounted

# Base class for settings-file migration chains. Mirrors SaveMigrations.
#
...
#
#     # In main.gd, before any Persister.load_settings / store_settings calls:
#     Persister.settings_migrations = KinspritSettingsMigrations.new()
#
...
func migrate(d: Dictionary) -> Dictionary:
	...
	if version > target:
		push_warning("SettingsMigrations.migrate: settings were written by a newer version (%d > %d). Loading as-is." % [version, target])
		return d
...
func _step(d: Dictionary, from_version: int) -> Dictionary:
	push_warning("SettingsMigrations: no migrator registered from version %d; stamping to %d and continuing" % [from_version, current_version()])
